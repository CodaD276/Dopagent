...
[快速開始](#快速開始) · [功能介紹](#這個-skill-能做什麼) · [架構](#架構層次) · [FAQ](#常見問題)
...
# 3. 引導 Agent
# 在 HanaAgent 聊天框中直接輸入：
# 「載入 dopagent skill」
#
# （HanaAgent 透過對話指令載入 skill——
#   輸入後會讀取 SKILL.md 並執行 bootstrap。）
```
...
```
1. 對 Agent 說一句明顯錯誤的話，例如「台南在台北」
2. Agent 糾正你之後，說「你記不記得我剛才糾正了什麼？」
3. Agent 應該能召回剛才的糾正內容
```

糾正→記憶→檢索——一整條回路，30 秒驗證。
...
L4-L5 不是缺失功能——框架已就緒，數據由日常糾正自然積累。無需手動觸發。
...
| 溫儲存 | 被多次驗證的規律，`hotness.py promote` 自動提煉並 retain |
...
A: Engagement 檢測已可用（✅）。糾正驗證需在 config.py 中配置 VERIFY_LLM_API_KEY 後啟用（⚙️）。

**Q: Hindsight 掛了，糾正內容會丟嗎？**  
A: 不會。框架自動暫存到熱儲存，等 Hindsight 恢復後同步。Agent 回覆不會因等待 Hindsight 而卡住（>10s 自動跳過）。

**Q: 糾正驗證的 API key 怎麼配？**  
A: 編輯 config.py，取消註釋並填入：
```python
VERIFY_LLM_API_KEY = "sk-your-key"
VERIFY_LLM_ENDPOINT = "https://api.openai.com/v1/chat/completions"  # 或其他相容 API
VERIFY_LLM_MODEL = "gpt-3.5-turbo"  # 越便宜越好，只做質檢
```
