...
新条目入库时检查标签重叠的已有条目 → 重叠 ≥2 标签 → 在热存储 footer 中双向标注 `see_also: <id>`。promote 时关联条目一起提交。

同一类型的纠正积累 ≥3 条 → Agent 判断是否构成稳定模式 → 是 → Hindsight retain (imp:6, type:pattern) + 热存储标注 [pattern]。否 → 继续积累。

分类维度（影响提炼精度）：`correction`（纠正）| `knowledge_gap`（知识空缺）| `best_practice`（最佳实践）| `decision`（架构决策）。纠正模板的"元模式"字段用于标注分类。
...
5. 行动（三选一，必答）：推 / 等 / 换
   → 选"推"时先 Alaya recall 任务关键词，以相关教训支撑提议
```
...
4. User 确认后 → Hindsight retain (imp:7, type:insight) + 写入热存储 [insight]，标注来源为 surfacing 信号
