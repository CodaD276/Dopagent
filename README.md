...
**Q: recall 超时？**  
A: Hindsight 的本地嵌入模型首次查询较慢（30-90 秒）。后续查询会快一些。如果持续超时，用 `--budget low` 缩小检索范围。

**Q: 我需要理解 Hindsight 或 Alaya 的原理吗？**  
A: 不需要。纠正 Agent → 自动学习。原理对日常使用透明。

**Q: 纠正验证 / Engagement 检测什么时候能用？**  
A: 🚧 规划设计完成，代码待实现。当前核心回路（纠正→retain→Alaya 召回）已完整可用。

## 规划中

| 功能 | 状态 |
|---|---|
| 温存储提炼（hot→warm） | 📐 设计完成，`hotness.py promote` 待实现 |
